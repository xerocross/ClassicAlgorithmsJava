...
/**
 * The class WeightedGraphAdj is a replacement
 ...
 *
 */
public class WeightedGraphAdj implements Graph, WeightedGraph
{
	...
	public WeightedGraphAdj(ArrayList<Vertex> vertices){
	...
	{
		if (!containsVertex(vA) || !containsVertex(vB))
			return false;
		WeightedEdge edge = getEdge(vA,vB);
	...
	{
		return removeEdge(edge.getLeft(), edge.getRight());
	}
	...
	{
		Set<Vertex> verticesCopy = new HashSet<Vertex>();
		for (Vertex v : vertices)
			verticesCopy.add(v);
		return verticesCopy;
	}
	...
	{
		addEdge(edge.getLeft(), edge.getRight(), 0.0);
	}
	...
	public Set<Edge> getEdges()
	{
		...
		Set<Edge> realEdges = getRealEdges();
		for (Edge re : realEdges)
		{
			edgesCopy.add(((WeightedEdge) re).clone());
		}
		return edgesCopy;
	}
	...
	{
		return getEdgeWeight(edge.getLeft(), edge.getRight());
	}
	public Double getSumEdgeWeights()
	{
		Double sumWeight = 0.;
		Set<Edge> edges = getRealEdges();
		for (Edge edge : edges)
		{
			sumWeight+= ((WeightedEdge)edge).getWeight();
		}
		return sumWeight;
	}
